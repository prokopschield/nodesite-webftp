...
function serveFile(fspath) {
    const root = path_1.default.resolve(config_1.folder);
    ...
    for (const h of config_1.hidden.array) {
        if (fspath.includes(path_1.default.resolve(config_1.folder, h.toString()))) {
    ...
    return ({
        statusCode: 200,
        head: {
            'Content-Type': mime_types_1.contentType(fn) || 'text/plain',
...
function serve(request) {
    ...
    let parts = request.uri.split(/[\\\/]+/g)
        ...
        .filter((a) => (a
        && !a.includes('..')));
    let fspath = path_1.default.resolve(config_1.folder);
